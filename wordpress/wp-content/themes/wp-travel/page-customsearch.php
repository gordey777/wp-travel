          ...
          <h1>Туры в кению</h1>
            ...
            <div class="form-wrap">
              <div class="input-wrapp">
                ...
                <input type="text" class="select__js" id="input-region" placeholder="Например “Мальдивы”" data-list="Габон, Гамбия, Гана, Гвинея, Гвинея-Бисау, Джибути, Замбия, Зимбабве" data-select="Ганна, Замбия">
              </div>

              <div class="input-wrapp">
                <label for="input-direction">Страны</label>
                <input type="text" class="select__js" id="input-country" placeholder="Например “Мальдивы”" data-list="Габон, Гамбия, Гана, Гвинея, Гвинея-Бисау, Джибути, Замбия, Зимбабве" data-select="Ганна, Замбия">
              </div>
...
echo get_cat_ID('Африка');

    if( $categories = get_categories( $args ) ) : // to make it simple I use default categories
      echo '<select id="regionfilter" name="regionfilter"><option value="">Select category...</option>';
      foreach ( $categories as $cat ) :
          $cat__ID = $cat->cat_ID;
          $field_term = 'category_' . $cat__ID;
          $cat__type = get_field('cat_type', $field_term);
if($cat__type === 'region') {
        echo '<option value="' . $cat->name . '">' . $cat->name . '</option>'; // ID of the category as the value of an option
}
...
<!-- <input type="text" id="regionfilter" name="regionfilter"> -->
  ...
  <?php
    if( $categories = get_categories( $args ) ) : // to make it simple I use default categories
      echo '<select id="countryfilter" name="countryfilter"><option value="">Select category...</option>';
      foreach ( $categories as $cat ) :
          $cat__ID = $cat->cat_ID;
          $field_term = 'category_' . $cat__ID;
          $cat__type = get_field('cat_type', $field_term);
